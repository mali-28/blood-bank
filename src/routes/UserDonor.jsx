...
const Userdonor = () => {
    ...
    useEffect(() => {
        if (showData[id] && isDonor ) {
            const currentUser = showData[id].userData;
            // console.log("grouo",currentUser.bloodGroup)
            setDonorData((pre)=>{
               return {...pre, name : currentUser.name, phone : currentUser.phone, bloodGroup : currentUser.bloodGroup}
            })
        }else{
            setDonorData((pre)=>{
        ...
        try {
            ...
            if (nameError !== "") {
                ...
                set(ref(db, 'users/' + user.id), {
                    userData: { id: user.id, name: donorData.name, email: user.mail, phone: donorData.phone, bloodGroup: donorData.bloodGroup, longitude, latitude },
                });
            ...
            console.log("errorabc",err.message)
        }
    }
    return <>
        ...
        <div className="w-40 box-shadow-ccc b-1-c9 p-4 m-3-auto d-flex flex-direction-column align-items-center">
            ...
            <span className='f-014 mb-2'>{message}</span>
            ...
            <div className="w-100per mt-1 d-flex flex-direction-row j-content-spacearound" onChange={change} >
                ...
                <div>
                    <label>
                    <input readOnly className="cursor-pointer mt-06 mr-02" type="radio" value="O" name="bloodGroup" checked={donorData.bloodGroup === "O"} /> O
                    </label>
                </div>
                <div>
                    <label>
                    <input readOnly className="cursor-pointer mt-06 mr-02" type="radio" value="A" name="bloodGroup" checked={donorData.bloodGroup === "A"}/> A
                    </label>
                </div>
                <div>
                    <label>
                    <input readOnly className="cursor-pointer mt-06 mr-02" type="radio" value="B" name="bloodGroup" checked={donorData.bloodGroup === "B"} /> B
                    </label>
                </div>
                <div>
                    <label>
                    <input readOnly className="cursor-pointer mt-06 mr-02" type="radio" value="AB" name="bloodGroup" checked={donorData.bloodGroup === "AB"} /> AB
                    </label>
                </div>
